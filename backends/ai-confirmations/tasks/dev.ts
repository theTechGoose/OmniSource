...
async function streamOutput(reader: ReadableStreamDefaultReader<Uint8Array>, writer: any, filter: (line: string) => boolean) {
...
function getGitRoot(): string {
  try {
    ...
    if (result.success) {
      ...
      throw new Error("Failed to locate Git root. Ensure you're in a Git repository.");
    }
  } catch (err) {
    //@ts-ignore
...
function buildServeCommand(): string[] {
  const ENV_FILE = "/Users/goose/Documents/New_Programing/OmniSource/.env.local";
...
const handleChange = debounce(async () => {
  ...
  console.clear();
  console.log("Detected change. Restarting processes...");
